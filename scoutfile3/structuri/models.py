...
STATUT_DREPTURI_CENTRU_LOCAL = (("depline", "Depline"), ("suspendat", "Suspendat"), ("propus_desfiintare", u"Propus spre desființare"), ("gi", "Grup de inițiativă"))
...
class CentruLocal(Structura):
    ...
    preferinte_corespondenta = models.CharField(max_length = 255, choices = TIPURI_CORESPONDENTA_CENTRU_LOCAL, default = "email", verbose_name = u"Preferință trimitere corespondență", help_text = u"Asigurați-vă că ați adăugat informațiile relevante de contact pentru tipul de corespondență ales.")
...
class Unitate(Structura):
    ...
    def total_membri_activi(self):
        return AsociereMembruStructura.objects.filter(content_type = ContentType.objects.get_for_model(self),
                                       object_id = self.id,
                                       tip_asociere__content_types__in = (ContentType.objects.get_for_model(self), ),
...
class Membru(Utilizator):
    ...
    def get_extra_badges(self):
        ...
        if self.afilieri_curente(end_chain = False).filter(tip_asociere__nume__icontains = u"Membru Consiliul Centrului Local").count():
    ...
    def rezerva_credit(self):
        ...
        if credit.credit_disponibil() > 0:
            ...
            rezervare = RezervareCredit(credit = credit, content_type = ContentType.objects.get_for_model(self), object_id = self.id)
            rezervare.save()
    ...
    def elibereaza_credit(self):
        from scoutfile3.patrocle.models import RezervareCredit
        rezervari = RezervareCredit.objects.filter(content_type = ContentType.objects.get_for_model(self), object_id = self.id)
...
class InformatieValabilaManager(models.Manager):
    def get_query_set(self):
        return super(InformatieValabilaManager, self).get_query_set().filter(Q(data_end__isnull = True) | Q(data_end__lte = datetime.datetime.now()))
