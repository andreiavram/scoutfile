...
class CentruLocalCreate(CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, u"Centrul Local a fost adăugat cu succes. Nu uita să adaugi și echipa de conducere!")
...
class CentruLocalDetail(DetailView, TabbedViewMixin):
    ...
    def get_tabs(self, *args, **kwargs):
        self.tabs = (("brief", u"Sumar", reverse("structuri:cl_tab_brief", kwargs = {"pk": self.object.id}), "", 2),
                ...
                ("contact", u"Contact", reverse("structuri:cl_tab_contact", kwargs = {"pk" : self.object.id }), "", 1),)
...
class CentruLocalLiderCreate(CreateView):
    ...
    def form_valid(self, form):
        ...
        info_kwargs = {"tip_informatie" : TipInformatieContact.objects.get(nume = u"Email", relevanta__icontains = u"Membru"), 
        ...
        info_kwargs.update({"tip_informatie" : TipInformatieContact.objects.get(nume = u"Adresă corespondență", relevanta__icontains = u"Membru"), 
                       "valoare" : self.object.adresa})
        InformatieContact(**info_kwargs).save()
        
        if self.object.telefon:
            info_kwargs.update({"tip_informatie" : TipInformatieContact.objects.get(nume = u"Mobil", relevanta__icontains = u"Membru"),
        ...
        messages.success(self.request, u"Adăugat cu succes pe %s, %s la %s" % (self.object, pozitie, self.get_unitate(form = form)))
        return HttpResponseRedirect(self.get_success_url())
    ...
    def create_associations(self, form):
        try:
            ...
            logger.error(u"%s: asocierea cu centrul local a esuat: %s %s" % (self.__class__.__name__, e, traceback.format_exc()))
        ...
        try:
            ...
            logger.error(u"%s: asocierea cu unitatea a esuat: %s %s" % (self.__class__.__name__, e, traceback.format_exc()))
...
class CentruLocalTabBrief(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        qs = AsociereMembruStructura.objects.filter(content_type = ContentType.objects.get_for_model(self.object), 
                                                   ...
                                                   tip_asociere__nume__iexact = u"Membru").order_by("membru__timestamp_confirmed")
...
class PatrulaUpdate(UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not membru.are_calitate(u"Membru Consiliu Centru Local", self.object.unitate.centru_local) and not membru.are_calitate(u"Lider", self.object.unitate):
            return HttpResponseRedirect(reverse("login") + "?unauthorized")        
...
class PatrulaTabMembri(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        tip_asociere_membru = TipAsociereMembruStructura.objects.get(nume = u"Membru", content_types__in = (ContentType.objects.get_for_model(Patrula),))
...
class PatrulaMembruCreate(CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        membru = request.user.get_profile().membru
        if not membru.are_calitate(u"Membru Consiliu Centru Local", self.patrula.unitate.centru_local) and not membru.are_calitate(u"Lider", self.patrula.unitate):
            return HttpResponseRedirect(reverse("login") + "?unauthorized")        
...
class MembruEditProfilePicture(UpdateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            logger.error(u"%s: nu am putut să uploades poza. %s : %s" % (self.__class__.__name__, e, traceback.format_exc()))
...
class ForgotPassword(FormView):
    ...
    def form_valid(self, form):
        ...
        if utilizator.timestamp_accepted == None:
            messages.error(self.request, u"Utilizatorul există, dar nu a fost încă confirmat de un lider. Așteaptă pentru când este confirmat, apoi solicită resetarea parolei!")
            return HttpResponseRedirect(reverse("structuri:membru_forgot_password"))
        ...
        if not DEBUG or USE_EMAIL_CONFIRMATION:
            message = render_to_string("structuri/inregistrare/password_reset.txt", {"utilizator" : utilizator, "form" : form}, context_instance = RequestContext(self.request))
...
class ConfirmForgotPassword(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not DEBUG or USE_EMAIL_CONFIRMATION:
            message = render_to_string("structuri/inregistrare/password_reset_confirm.txt", {"utilizator" : self.utilizator, "password" : new_password}, context_instance = RequestContext(request))
            send_mail("Parola noua pentru ScoutFile", message, SYSTEM_EMAIL, [self.utilizator.email, ])
        
        messages.success(request, u"O nouă parola a fost generată și trimisă pe adresa ta de email, %s" % self.utilizator.email)
        return HttpResponseRedirect(reverse("login"))
...
class RegisterMembru(CreateView):
    ...
    def form_valid(self, form):
        ...
        asociere = AsociereMembruStructura(membru = self.object,
                                           content_type = ContentType.objects.get_for_model(form.cleaned_data['centrul_local']),
        ...
        if not DEBUG or USE_EMAIL_CONFIRMATION:
            ...
            body = render_to_string("structuri/inregistrare/registration_email.txt", {"utilizator" : self.object, "centru_local" : form.cleaned_data['centrul_local'], "unitate" : form.cleaned_data['unitate']}, context_instance = RequestContext(self.request))
            send_mail("Inregistrare ScoutFile", body, SYSTEM_EMAIL, [self.object.email, ])
      
        messages.success(self.request, u"Solicitarea ta a fost trimisă, vei primi un email cu un link pentru confirmare!")  
        return HttpResponseRedirect(self.get_success_url())
...
class ConfirmMembruAdmin(UpdateView):
    ...
    def form_valid(self, form):
        ...
        if "reject" in self.request.POST:
            ...
            message = render_to_string("structuri/inregistrare/registration_deny.txt", {"membru" : self.object}, context_instance = RequestContext(self.request)),
            ...
            return HttpResponseRedirect(reverse("structuri:cl_membri_pending", kwargs = {"pk" : self.request.user.get_profile().membru.centru_local.id}))
        ...
        if not DEBUG or USE_EMAIL_CONFIRMATION:
            ...
            send_mail(u"Confirmare acceptare cont pe ScoutFile", 
                      render_to_string("structuri/inregistrare/registration_accept.txt", {"membru" : self.object}, context_instance = RequestContext(self.request)), 
                      SYSTEM_EMAIL,
                      [self.object.email, ])
...
class AsociereUpdate(UpdateView):
    ...
    @allow_by_afiliere([("Afiliere, Membru, *, Centru Local", "Lider"), ("Afiliere, Membru, *, Centru Local", "Membru Consiliul Centrului Local")])
    def dispatch(self, request, *args, **kwargs):
...
class MembruContactUpdate(ContactUpdate):
    ...
    @allow_by_afiliere([("InformatieContact, Membru, Centru Local", "Lider"), ("InformatieContact, Membru, Centru Local", "Membru Consiliul Centrului Local")])
    def dispatch(self, request, *args, **kwargs):
...
class MembriForPatrocle(View):
    def get(self, request, *args, **kwargs):
        ...
        qs = qs.filter(afilieri__content_type = ContentType.objects.get_for_model(CentruLocal), afilieri__object_id = self.request.user.get_profile().membru.get_centru_local().id, afilieri__tip_asociere__nume__iexact = u"Membru", afilieri__moment_incheiere__isnull = True).distinct()
        ...
        qs = qs.filter(membru__afilieri__content_type = ContentType.objects.get_for_model(CentruLocal), membru__afilieri__object_id = self.request.user.get_profile().membru.get_centru_local().id, membru__afilieri__tip_asociere__nume__iexact = u"Membru", membru__afilieri__moment_incheiere__isnull = True)        
...
class GetSpeedList(View):
    def handler_ccl(self, request, *args, **kwargs):
        ...
        return [[filter_mobil(m.informatie_contact("mobil")), "%s" % m.id] for m in data if filter_mobil(m.informatie_contact("mobil")) != ""]
