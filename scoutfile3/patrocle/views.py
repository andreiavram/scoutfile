...
#             return HttpResponseRedirect(reverse("patrocle:home"))
        ...
        if len(form.cleaned_data['destinatari']) > 1:
            grup_id = "%d-%s" % (self.request.user.get_profile().id, hashlib.md5("%s" % datetime.datetime.now()).hexdigest())
            
            
        for d in form.cleaned_data['destinatari']:
            try:
                SMSMessage.send_message("sms",
                                        expeditor = self.request.user.get_profile(), 
                                        destinatar = d[0], 
...
class ListConfirmations(ListView):
    ...
    def get_queryset(self):
        return super(ListConfirmations, self).get_queryset().filter(expeditor = self.request.user.get_profile()).order_by("confirmat", "-timestamp_trimitere")
...
class ListGrupConfirmations(ListView):
    ...
    def get_queryset(self):
        filter_kwargs = { 'expeditor' : self.request.user.get_profile(),
...
class AsociazaCredit(CreateView):
    ...
    def form_valid(self, form):
        ...
        self.object.creat_de = self.request.user.get_profile()
        self.object.tip = 2
        self.object.save()
